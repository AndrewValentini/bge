...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data bge.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data bge.awk)" | ${PAGER}
	done
	exit 0
fi

get_data bge.tsv 2>/dev/null | awk -v cmd=ref -v ref="$*" "$(get_data bge.awk)" 2>/dev/null | ${PAGER}
